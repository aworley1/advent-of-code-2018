...
class Board(
    val rows: List<Row>,
    val cloths: MutableList<Cloth> = mutableListOf()
) {
    ...
    companion object {
        ...
        fun buildEmptyRow(noCols: Int): Row {
            return Row(List(noCols) { Square(Status.EMPTY) })
        }
    }

    fun addToBoard(parsedInput: ParsedInput) {
        cloths.add(parsedInput.cloth)
        for (col in parsedInput.startingCoords.column..parsedInput.startingCoords.column + parsedInput.sizeInCols - 1) {
            for (row in parsedInput.startingCoords.row..parsedInput.startingCoords.row + parsedInput.sizeInRows - 1) {
                ...
                parsedInput.cloth.add(this[row][col])
            }
        }
    }
    ...
    fun findClothsWithoutOverlaps(): String {
        return cloths.map { it.id to it.squares.filter { it.status == Status.MULTI }.count() }
            .filter { it.second == 0 }
            .single()
            .first
    }
...
class Cloth(val id: String, val squares: MutableSet<Square> = mutableSetOf()) {
...
class Square(var status: Status) {
    fun putClothOn() {
...
fun parseInput(input: String): ParsedInput {
    ...
    val sizeOfClothString = splitBySpace[3]
    val sizeOfCloth = sizeOfClothString.split("x")
    val clothSizeCols = sizeOfCloth[0].toInt()
    val clothSizeRows = sizeOfCloth[1].toInt()

    val clothId = splitBySpace[0]


    return ParsedInput(startSquareCoords, clothSizeCols, clothSizeRows, Cloth(clothId))
}

class Coords(val column: Int, val row: Int)

class ParsedInput(val startingCoords: Coords, val sizeInCols: Int, val sizeInRows: Int, val cloth: Cloth)
...
fun main() {
    ...
    println(board.findClothsWithoutOverlaps())
