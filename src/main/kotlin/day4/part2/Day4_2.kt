...
fun main() {
    ...
    val guardsAndMinutesAsleep = shifts
        .groupBy { it.guardId }
        .map { it.key to it.value.flatMap { it.sleepPeriods.flatMap { it.getMinutesInSleepPeriod() } } }
        .map { it.first to it.second.groupBy { it }.map { it.key to it.value.size } }

    val maxInstancesOfAMinuteForEachGuard =
        guardsAndMinutesAsleep.map { it.first to it.second.maxBy { it.second } }
            .filter { it.second != null }

    val maxGuard = maxInstancesOfAMinuteForEachGuard.maxBy { it.second!!.second }!!

    println(maxGuard)
    println(maxGuard.first.replace("#", "").toInt() * maxGuard.second!!.first)
